...
MODEL_DIR = osp.join(models_path, "insightface")

def download(sub_dir, name, force=False, root=MODEL_DIR):
...
def ensure_available(sub_dir, name, root=MODEL_DIR):
    return download(sub_dir, name, force=False, root=root)

def download_onnx(sub_dir, model_file, force=False, root=MODEL_DIR, download_zip=False):
    _root = os.path.expanduser(root)
