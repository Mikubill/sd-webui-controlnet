...
from scripts.hook import th, ControlParams
from scripts.hook import UnetHook as UnetHook_cn


class UnetHook(UnetHook_cn):
    ...
    def hook(self, model):
        ...
        def forward(self, x, timesteps=None, context=None, **kwargs):
            ...
            # Plugin cress attention map for PwW 
            is_pww_init = outer.p and len(outer.pww_cross_attention_weight) > 0
            if  is_pww_init and context is not None:
                sigma = outer.p.sampler.get_sigmas(outer.p, 1000)[-int(timesteps[0])]
                context = {'CONTEXT_TENSOR': context, 'SIGMA':sigma}
                context.update(outer.pww_cross_attention_weight)
